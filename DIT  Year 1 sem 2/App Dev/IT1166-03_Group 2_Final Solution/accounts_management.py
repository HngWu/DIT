...
class Account:
    ...
    Attributes:
        user_id (str): The unique identifier of the account.
...
class AccountsManagement:
        ...
        with shelve.open(self.db_path) as db:
            for account in db.values():
                if account.email == email and account.password == password:
                    print("user id: " + str(account.user_id))
                    return account.user_id
        return None

    def login_as_admin(self, email: str, password: str) -> str:
        """
        Perform login with email and password.
        ...
        with shelve.open(self.db_path) as db:
            for account in db.values():
                ...
                print(account.admin_status)
                if (
                    account.email == email and account.password == password
                ) and account.admin_status:
    ...
    def get_all_accounts(self) -> list:
        """
        Retrieve all accounts and format them into a list of dictionaries.
                ...
                accounts_list.append(
                    {
                        ...
                        "Date of Birth": account.date_of_birth.isoformat(),
                        "Email": account.email,
                        "Gender": ["Male", "Female", "Other"][account.gender],
                        "Receive Newsletters": (
                            "Yes" if account.receive_newsletters else "No"
                        ),
                        "Admin Status": "Yes" if account.admin_status else "No",
                    }
                )
